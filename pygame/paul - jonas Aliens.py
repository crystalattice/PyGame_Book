...
ALIEN_ODDS     = 0.2     #chances a new alien appears
BOMB_ODDS      = 10    #chances a new bomb will drop
ALIEN_RELOAD   = 18     #frames between new aliens
SCREENRECT     = pygame.Rect(0, 0, 2000, 800)
SCORE          = 0
Kurt_ODDS      = 0,3
Kurt_RELOAD    = 10
...
class Kurt(pygame.sprite.Sprite):
    speed = 20
    animcycle = 1 
    images = []
    def __init__(self):
        ...
        self.image = self.images[0]
        self.rect = self.image.get_rect()
        self.facing = random.choice((-1,1)) * Kurt.speed
        self.frame = 0
        if self.facing < 0:
            self.rect.right = SCREENRECT.right
...
def main(winstyle = 0):
    ...
    Kurt.images = load_images('alien1.gif', 'alien2.gif', 'alien3.gif')
    ...
    Kurt.containers = aliens, all, lastalien
    ...
    while player.alive():
        ...
        # Create new alien
        if alienreload:
            alienreload = alienreload - 2
        elif not int(random.random() * ALIEN_ODDS):
            Alien()
            alienreload = ALIEN_RELOAD
 
        if alienreload:
            Kurtreload = Kurtreload - 2
        elif not int(random.random() * ALIEN_ODDS):
            Kurt()
            kurtreload = Kurt_RELOAD
        ...
        for alien in list(pygame.sprite.groupcollide(shots, aliens, 1, 1).keys()):
            boom_sound.play()
            Explosion(alien)
            SCORE = SCORE + 100
            if pygame.font:
